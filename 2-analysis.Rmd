...
```{r plot.durations}
plotFirstOccIndivJoint(surges.scp)
```

```{r durations.save, echo=savePaperGraphs, eval=savePaperGraphs}
ggsave('durations.pdf',g,width=20,height=12,unit='cm')
```

### Combination 2

```{r plot.combi2}
g3 <- plotFirstOccIndivJoint(surges.scp)
g <- doublePlotAcrossTime(joint, surges.scp,addThird = g3)
```

```{r combi2.save, echo=savePaperGraphs, eval=savePaperGraphs}
...
```{r eval.create.data.to.annotate1,eval=!shortVersion}
...
```{r eval.create.data.to.annotate2,eval=!shortVersion}
set.seed(2022) 
...
```{r eval.create.data.to.annotate3,eval=!shortVersion}
...
```{r eval.results1,eval=!shortVersion}
