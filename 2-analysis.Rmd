...
The representation of all the points is very computationally heavy, causing too long duration even to load the pdf file. This is why a random subset of 10,000 relations is used. Note that the random selection can affect the graph quite significantly since the extreme points are rare. 

```{r rnd.subset.ma}
relations.10k<-pickRandomDynamic(relations.ma,n=1000)

```

```{r trend.distrib3}
#displayTrendDistribution2(relations.ma,indiv.ma,withRect = FALSE)
g<-displayTwoPlotsDistribTrend(relations.10k,indiv.ma)
g
```
...
```{r eval.years}
doublePlotAcrossTime(dynamic_joint, surges,bins=68,fontsize=16)
```


```{r eval.years.threeplots}
g<-threePlotsAboutYears(surges, dynamic_indiv, dynamic_joint)
g
```
