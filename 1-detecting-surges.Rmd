...
```{r trend}
computeTrend(rel.measures, indicator='rate', measure='nmi')
```
...
```{r surges}
detectSurges(rel.measures, globalOutliers=FALSE)
```
...
```{r}
local_stats <- countSurgesByRelation(rel.measures)
...
```{r global.surges}
rel.mesures.global<-addDynamicAssociationToRelations(relations.ma,indiv.ma,measures = c('prob.joint','pmi','nmi'))
computeTrend(rel.mesures.global, indicator='rate', measure='nmi')
detectSurges(rel.mesures.global, globalOutliers=TRUE)
```


```{r}
global_stats <- countSurgesByRelation(rel.mesures.global)
...
computeAndSaveSurgesData(dataPath,outputFilePrefix=paste0(dataPath,'/'), ma_windows=c(1,3,5),indicators=c('rate','diff'), outlier_methods=c('local','global'))
```
