...
```{r measures}
rel.mesures<-addDynamicAssociationToRelations(relations.ma,indiv.ma,measures = c('prob.joint','pmi','npmi','mi', 'nmi'))
```
...
```{r surges}
local_surges <- detectSurges(rel.mesures, globalOutliers=FALSE)
global_surges <- detectSurges(rel.mesures, globalOutliers=TRUE)
```
