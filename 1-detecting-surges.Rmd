...
This document was generated from an [R Markdown](https://rmarkdown.rstudio.com/) source file. The source file is provided in the repository, allowing full reproducibility of the experiments presented in the paper.

This R Markdown document can be configured by modifying the following lines:
...
- `cowplot`
- `scales`
- `knitr` and `rmarkdown` (only if executing the Rmd source files)
...
This step is optional and takes a long time (5 to 10 minutes for the ND subset).
...
system.time(computeAndSaveSurgesData(dataPath,outputFilePrefix=paste0(dataPath,'/'), ma_windows=c(1,3,5),measures=c('prob.joint','pmi','npmi','mi','nmi','scp'),indicators=c('rate','diff')))
```
