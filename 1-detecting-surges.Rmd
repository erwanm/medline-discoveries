...
* Given a relation between two concepts c1 and c2 which appear for the ﬁrst time at years $y_1$ and $y_2$ respectively, the earliest possible
year for a cooccurrence (and consequently for a surge) is $max(y_1 , y_2)$. This can be calculated and added to the data table (column `year.first.both`) as follows. The difference between the surge year and this year is added as column `duration`.
